...
class SplitChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        freq_attention = self.layer_frequency(squeeze)
        spatial_attention = self.layer_spatial(squeeze)
...
class NLLP(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        super(NLLP, self).__init__()
...
class AMC(nn.Module):
    def __init__(self, in_channel=128, out_channel=128, kernel_size=3, stride=1, padding_mode='reflection', need_bias=True):
        super(AMC, self).__init__()
        self.sca = SplitChannelAttention(channel=in_channel, ratio=16)

        # 左侧卷积分支
        self.left_conv = nn.Conv2d(in_channel, in_channel, kernel_size=3, padding=1)
        # 池化、反卷积、上采样分支
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.deconv = nn.ConvTranspose2d(in_channel, in_channel, kernel_size=2, stride=2)
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.right_conv = nn.Conv2d(in_channel, in_channel, kernel_size=3, padding=1)

        self.fu = FourierUnit(in_channel, out_channel)

        self.nllp = NLLP(in_channel * 2, in_channel, in_channel)
    ...
    def forward(self, x):
        ...
        # 左侧卷积分支
        left_out = self.left_conv(x)
        # 池化、反卷积、上采样分支
        pool_out = self.pool(x)
        deconv_out = self.deconv(pool_out)
        upsample_out = self.upsample(deconv_out)
        right_out = self.right_conv(upsample_out)

        # 整合两个分支输出
        branch_out = torch.cat([left_out, right_out], dim=1)
        nllp_out = self.nllp(branch_out)
        softmax_out = self.softmax(nllp_out)
        ...
        fu_out = self.fu(x_f)
        # 结合各部分输出
        final_out = fu_out + x_s + self.final_conv(softmax_out)
        return final_out
...
class TextureTransformer(nn.Module):
    ...
    def forward(self, l0, m1_up, l0_down, m1):
        ...
        F_main = self.norm1(self.f_extractor(m1))  # [8,128,16,16]
...
class BasicBlock(torch.nn.Module):
    ...
    def forward(self, x, PhiWeight, z, q):
        ...
        out4 = self.relu(self.ttf(r_out, r_out, r_out, out2))+out3

        out5 = self.ttf(out4) # 8 128 16 16

        out5 = (self.ttf(out3,out2,out2,out4)) + out5  # 8 128 16 16
